...
HERE;
	...
	function getHTML_ReferrersRecent() 
	{
		...
		// Referrers Pane
		$query = "SELECT `referer`, `resource`, `resource_title`, `search_terms`, `img_search_found`, `dt`
					FROM `{$this->Mint->db['tblPrefix']}visit` 
	...
	function getHTML_PagesRecent() 
	{
		...
		if (isset($this->Mint->cfg['manifest']['visit']['referred_by_feed']))
		{
			$from_feed = ", `referred_by_feed`";
		}
		
		$query = "SELECT `referer`, `resource`, `resource_checksum`, `resource_title`, `search_terms`, `img_search_found`, `dt`{$from_feed}
					FROM `{$this->Mint->db['tblPrefix']}visit` 
					ORDER BY `dt` DESC 
					LIMIT 0,{$this->Mint->cfg['preferences']['rows']}";
